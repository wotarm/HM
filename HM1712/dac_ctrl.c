...
{
		if(DAC_InitStruct->instance == HW_DAC0)
		{
			SIM->SCGC2 |= SIM_SCGC2_DAC0_MASK;
		}
		if(DAC_InitStruct->instance == HW_DAC1)
		{
...
uint16_t DAC_GetBufferValue(uint32_t instance,uint8_t i)
{
	return DAC_InstanceTable[instance]->DAT[i].DATL + (DAC_InstanceTable[instance]->DAT[i].DATH<<8);
}
/**
 * @brief  注册中断回调函数
...
{
    DAC_InitTypeDef DAC_InitStruct = {0};
    DAC_InitStruct.bufferMode = kDAC_Buffer_Disable; /*Buffer????  */
    DAC_InitStruct.instance = HW_DAC0;
    DAC_InitStruct.referenceMode = kDAC_Reference_1; /* VREFOUT */
    DAC_InitStruct.triggerMode = kDAC_TriggerSoftware; /*???? */
    DAC_Init(&DAC_InitStruct);
		
		DAC_InitStruct.bufferMode = kDAC_Buffer_Disable; 
    DAC_InitStruct.instance = HW_DAC1;
    DAC_InitStruct.referenceMode = kDAC_Reference_1; 
    DAC_InitStruct.triggerMode = kDAC_TriggerSoftware; 
    DAC_Init(&DAC_InitStruct);
}

unsigned int readDAC(unsigned char HW)
{
	return DAC_GetBufferValue(HW, 0);
}
extern void SendStr(unsigned char* dat, char channel);
void writeDAC(unsigned char HW, unsigned short value)
{
	//	char buf[3];
	//	sprintf(buf,"%d",value);
	//SendStr((unsigned char*)buf,0);
    DAC_SetBufferValue(HW, &value, 1);
		if(1){
			if(value<=1410){
			//	SendStr((unsigned char*)"set ------0",0);
				GPIO_WriteBit(HW_GPIOB, 2,0);      //turn the las pin
				
			}
			else{
			//		SendStr((unsigned char*)"set ------1",0);
				GPIO_WriteBit(HW_GPIOB, 2,1);      //turn the las pin
			}
		}
    //DAC_SoftwareStartConversion(HW);
}
