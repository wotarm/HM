...
{
    { 10L,          spi_task,  1500L,  8L,         "spi", MQX_AUTO_START_TASK, 0,     0  },
    { 0 }
};
*/
...
void FPGA_init()
{
    HW_ptr.spifp = fopen (TEST_CHANNEL, NULL);
		if (NULL == HW_ptr.spifp)
    {
        printf ("Error opening SPI driver!\n");
        _time_delay (200L);
        //_task_block ();
    ...
    param = 500000;
    printf ("Changing the baud rate to %d Hz ... ", param);
    if (SPI_OK == ioctl (HW_ptr.spifp, IO_IOCTL_SPI_SET_BAUD, &param))
    {
        printf ("OK\n");
    }
    else
    ...
    param = SPI_CLK_POL_PHA_MODE0;
    printf ("Setting clock mode to %s ... ", clock_mode[param]);
    if (SPI_OK == ioctl (HW_ptr.spifp, IO_IOCTL_SPI_SET_MODE, &param))
    {
        printf ("OK\n");
    }
    else
    ...
    param = SPI_DEVICE_MASTER_MODE;
    printf ("Setting transfer mode to %s ... ", device_mode[param]);
    if (SPI_OK == ioctl (HW_ptr.spifp, IO_IOCTL_SPI_SET_TRANSFER_MODE, &param))
    {
        printf ("OK\n");
    }
    else
    {
		...
		if( SPI_OK == ioctl(HW_ptr.spifp, IO_IOCTL_SPI_SET_FLAGS, &param))
		{
			printf("Set full duplex OK\n");
		}
		else
		{
			printf("Set full duplex ERROR\n");
		}
		FPGA_reset();
...
unsigned int readFPGA(unsigned char addr)
{
    SPI_READ_WRITE_STRUCT  rw;
		char send_buffer[4];
		char recv_buffer[4];
		send_buffer[0] = addr|RFPGA;
    rw.BUFFER_LENGTH = 3;
    rw.WRITE_BUFFER = (char *)send_buffer;
    rw.READ_BUFFER = (char *)recv_buffer;
    //printf ("IO_IOCTL_SPI_READ_WRITE ... ");
		while(fpga_rw_flag == 1)
		{
			_time_delay(1);
		}
		GPIO_WriteBit(HW_GPIOC, 4, 0);
		fpga_rw_flag = 1;
    if (SPI_OK == ioctl (HW_ptr.spifp, IO_IOCTL_SPI_READ_WRITE, &rw))
    {
        //printf ("OK\n");
    }
    else
    {
        printf ("IO_IOCTL_SPI_READ_WRITE ERROR\n");
    }
    fflush (HW_ptr.spifp);
		ioctl(HW_ptr.spifp, IO_IOCTL_SPI_FLUSH_DEASSERT_CS, NULL);
		GPIO_WriteBit(HW_GPIOC, 4, 1);
		fpga_rw_flag = 0;
		return recv_buffer[2] + (recv_buffer[1]<<8);
}
